    ...
    if (dineroCliente >= dinero) 
    {
        if(total >= dinero)
        {
            ...
            if (dinero == 0)
            {
                ...
                resultado.innerHTML += "Se han retirado " + usuario.value + " Bolívares <hr />";
...
function estadoCuenta() {
    ...
    for (var c of clientela)
    {
        if (respuesta[0] == c.nombre && respuesta[1] == c.contraseña)
        {
            datos = '<p>Titular de la cuenta: ' + c.nombre + '</p><p>Número de cuenta: ' + c.numeroCuenta + '</p><p>Saldo de la cuenta: <b>' + c.dineroTotal + ' Bolívares</b></p>';
        }
    }
    swal.fire({
        customClass: {
...
function alertas() {
    ...
    await swal.fire({
        ...
        title: '¡Bienvenido al banco Bolivariano!', 
        text: '¿Ya tienes una cuenta?',
        grow: 'fullscreen',
        imageUrl: 'money-bag.svg',
        imageHeight: 250,
                ...
                if (result.isConfirmed) {
                    swal.mixin({ 
                        ...
                        },
                        {
                            ...
                            inputValidator: nombre => {
                                if (parseInt(nombre) < 50 || !nombre) {
                                    return "El deposito mínimo para abir tu cuenta es de 50 Bolívares.";
                                } else {
                                    return undefined;
                                }
                            }
                        ...
                        else
                        {
                            swal.fire({
                                ...
                                text: 'Ahora, disfruta de todos los beneficios de ser cliente del Banco Bolivariano.',
                                icon: 'success',
...
function cerrarSesion()
{
    if (totalEntregado > 0 && main.style.display == "block") {
        ...
        for(var c of clientela)
        {
            if (respuesta[0] == c.nombre && respuesta[1] == c.contraseña)
            {
                saldo = 'Gracias por preferir al Banco Bolivariano.<p>El saldo actual de su cuenta es: <b>' + c.dineroTotal + ' Bolívares.</b></p>';
            }
        }
        swal.fire({
            customClass: {
    ...
    swal.fire({
        ...
        for(var c of clientela)
        {
            if (respuesta[0] == c.nombre && respuesta[1] == c.contraseña)
            {
                ...
                saldo = '<p>El saldo actual de su cuenta es: <b>' + c.dineroTotal + ' Bolívares.</b></p>';
